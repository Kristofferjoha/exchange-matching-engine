...
impl SimLogger for NaiveFileWriteLogger {
    fn log_order_submission(&mut self, order: &Order) {
        if let Ok(writer) = &mut self.writer {
            let _ = writeln!(writer, "SUBMIT: id={}, side={:?}, qty={}, price={:?}", order.order_id, order.side, order.quantity, order.price.unwrap_or_default());
        }
    }
    fn log_trade(&mut self, trade: &Trade) {
        if let Ok(writer) = &mut self.writer {
            let _ = writeln!(writer, "TRADE: trade_id={}, price={}, qty={}", trade.trade_id, trade.price, trade.quantity);
        }
    }
    fn log_order_cancel(&mut self, order_id: &Uuid, success: bool) {
        if let Ok(writer) = &mut self.writer {
            let _ = writeln!(writer, "CANCEL: id={}, success={}", order_id, success);
        }
    }
    fn log_order_filled(&mut self, order: &Order) {
        if let Ok(writer) = &mut self.writer {
            let _ = writeln!(writer, "FILLED: id={}, final_status={:?}", order.order_id, order.status);
        }
    }
